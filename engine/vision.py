...
New signals vs old VisionEngine:
  sig_image    now (object, int, int)  — PIL image + batch_index + gen_id
  sig_progress (int, int, int)         — (current_step, total_steps, gen_id)
  sig_resource (dict)                  — {"vram_used_mb": int, "vram_free_mb": int}

The trailing gen_id on sig_image / sig_progress lets downstream consumers
correlate emissions to the specific generate() call that produced them —
needed so chat-side bridges can match incoming images to pending bubbles
without confusing them with concurrent VISION-tab user generations. Slots
with fewer params still work; Qt drops extra args.
...
class VisionProcess(EngineProcess):
    ...
    # ── signals ────────────────────────────────────────────────────────────
    sig_image    = Signal(object, int, int)   # (PIL.Image, batch_index, gen_id)
    sig_finished = Signal()
    sig_progress = Signal(int, int, int)      # (current_step, total_steps, gen_id)
    ...
    def reserve_gen_id(self) -> int:
        """Atomically allocate the next gen_id without submitting work.

        The skill executor calls this BEFORE submitting a generate op so it
        can register the gen_id with VisionArtifactBridge.register_pending()
        ahead of any sig_image / sig_progress arrival. Without reservation,
        there's a race between submit-and-engine-assigning-gen_id and the
        first progress callback the engine emits.
        """
        self._gen_id += 1
        return self._gen_id
    ...
    def generate(self, payload: dict) -> None:
        ...
        # If the caller (skill executor) pre-reserved a gen_id via
        # reserve_gen_id(), honor it. Otherwise auto-increment as before
        # (covers the VISION-tab user-click path which never reserves).
        reserved = payload.get("gen_id") if isinstance(payload, dict) else None
        if isinstance(reserved, int) and reserved > 0:
            self._active_gen_id = reserved
        else:
            self._gen_id += 1
        ...
        self._send("generate", gen_id=self._active_gen_id, config=full_cfg)
    ...
    def _dispatch_event(self, event: dict) -> None:
        ...
        if kind == "result":
            gid = int(event.get("gen_id") or 0)
            if gid == self._active_gen_id:
                img = event.get("image")
                idx = int(event.get("batch_index") or 0)
                if img is not None:
                    self.sig_image.emit(img, idx, gid)
