...
# Prevent diffusers/transformers from hanging on HuggingFace Hub downloads
...
def _load_pipeline(model_path: str, backend: str, dtype):
    ...
    if is_file:
        # Try local-only first to avoid hanging on HuggingFace downloads.
        # Fall back without local_files_only if local config isn't cached yet.
        try:
            return Cls.from_single_file(model_path, local_files_only=True, **kwargs)
        except Exception:
            # local_files_only failed — retry allowing network (env vars still
            # set HF_HUB_OFFLINE so this won't actually download, but diffusers
            # may fall back to a built-in default config instead of crashing).
