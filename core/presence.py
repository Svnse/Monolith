...
class PresenceEngine:
    ...
    def _behavioral_data(self, data: dict) -> dict:
        ...
        for key, value in data.items():
            if key in _IGNORED_KEYS:
                continue
            result[key] = self._sanitize_node(value, key_name=str(key))
        return result

    def _sanitize_node(self, node, key_name: str | None = None):
        if key_name and self._is_sensitive_key(key_name):
            return _REDACTED

        if isinstance(node, dict):
            sanitized: dict = {}
            for key, value in node.items():
                sanitized[key] = self._sanitize_node(value, key_name=str(key))
            return sanitized

        if isinstance(node, list):
            return [self._sanitize_node(item) for item in node]
    ...
    def _is_sensitive_key(self, key_name: str) -> bool:
        ...
        if compact in _SENSITIVE_NAMES:
            return True
    ...
    def _compare_nodes(self, old_node, new_node, path: str, out: dict) -> None:
        ...
        if isinstance(old_node, list) and isinstance(new_node, list):
            if len(old_node) != len(new_node):
                ...
                old_tail = deepcopy(old_node[len(new_node):]) if len(old_node) > len(new_node) else []
                out[length_path] = {
                    "old": len(old_node),
                    "new": len(new_node),
                    "old_tail": old_tail,
    ...
    def _apply_reverse_diff(self, operator_data: dict, diff: dict) -> None:
        for dotpath in sorted(diff.keys(), key=lambda item: item.count("."), reverse=True):
            ...
            if dotpath.endswith(".length"):
                list_path = dotpath[:-7]
                self._set_list_length(operator_data, list_path, change)
                continue
            self._set_dotpath_value(operator_data, dotpath, old_value)

    def _set_list_length(self, data: dict, dotpath: str, change: dict) -> None:
        length = change.get("old")
        if not isinstance(length, int) or length < 0:
            return
        target = self._get_dotpath_value(data, dotpath)
        if not isinstance(target, list):
            return
        old_tail = change.get("old_tail")
        if not isinstance(old_tail, list):
            old_tail = []
        while len(target) > length:
            target.pop()
        tail_index = 0
        while len(target) < length:
            if tail_index < len(old_tail):
                target.append(deepcopy(old_tail[tail_index]))
                tail_index += 1
            else:
                target.append(None)
