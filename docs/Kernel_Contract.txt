MONOLITH KERNEL CONTRACT — v2 (FROZEN) + v2.1 ADDENDUM (TASK-BOUND AUTHORIZATION)
...
SIGNAL PASSTHROUGH LAW:
Every command string in ENGINE_DISPATCH must have a matching method on EngineBridge.
If guard.py dispatches to it, bridge.py must forward it.
Violation = silent permanent queue deadlock.
When adding any new command: update ENGINE_DISPATCH, add handler on LLMEngine, and add passthrough on EngineBridge.
All three or none.
...
MonoBridge and MonoDock are non-authoritative.
They translate, organize, and route.
They do not execute or decide.
...
Cancel      Task/addon   Prevents queued execution; active execution may be stopped by Dock→Guard
STOP        Engine/all   Interrupts active execution immediately; clears active task; cancels queued tasks for target
...
- Requires policy interpretation beyond mechanical dispatch/state
...
============================================================
9. Task-Bound Authorization (v2.1 ADDENDUM)
============================================================

Purpose:
Eliminate dual-authority authorization failures by making MonoGuard the single source of truth
for task-bound authorization membership. This addendum adds only mechanical state and checks.
Policy and semantics remain outside the kernel.

------------------------------------------------------------
9.1 Authority Clarification
------------------------------------------------------------

MonoGuard is the sole authority for execution state and state transitions.
MonoGuard additionally owns authorization truth required to execute tool actions within an active Task,
as mechanical membership only (boolean check). MonoGuard does not interpret scope meaning.

------------------------------------------------------------
9.2 New MonoGuard State
------------------------------------------------------------

task_permissions: dict[str, dict[str, set[str]]]
# engine_key → task_id → set of opaque scope strings

Scope strings are opaque tokens.
MonoGuard does not parse, interpret, normalize, or expand scopes.
Scope comparison is exact equality only.
No hierarchy, wildcards, prefix matching, or structure is guaranteed by the kernel.

------------------------------------------------------------
9.3 New MonoGuard Interface
------------------------------------------------------------

def grant(engine_key: str, task_id: str, scope: str) -> None:
    Adds `scope` to task_permissions[engine_key][task_id].
    Grants for inactive tasks are permitted and inert until activation.

def check(engine_key: str, scope: str) -> bool:
    Returns True iff:
      1) engine_key has an active task in MonoGuard, and
      2) `scope` exists in the permission set for that active task_id.

    Callers must not supply task_id; MonoGuard derives it from active_tasks[engine_key].
    If no active task exists: returns False.

def clear(engine_key: str, task_id: str) -> None:
    Removes all scopes for (engine_key, task_id) if present.

------------------------------------------------------------
9.4 Invariants (Kernel-Enforced)
------------------------------------------------------------

1) Active-task binding
Authorization is evaluated only against the currently active task for the target engine.
Non-active tasks never authorize actions.

2) STOP cleanup
On STOP(target), MonoGuard MUST clear permissions for the active task(s) it clears, as part of clearing active work.

3) Completion cleanup
When an active task finishes (DONE/FAILED/ERROR), MonoGuard MUST clear that task’s permissions for that engine.

4) Inert pre-staging persistence
Permissions granted to tasks that never become active remain inert and persist until explicit clear() or process termination.
MonoGuard does not track queue lifecycle and does not clear permissions for never-activated queued tasks automatically.
...
- time-based expiration / timeouts
- leases
- revocation semantics beyond explicit clear()
- provenance, audit trails, or history
- policy evaluation

Any temporal or economic logic remains outside the kernel.
